...
def gconnect():
    # Validate state token what client sent to server is same as what server sent to client to emsure user is making the request and not malcious script
    if request.args.get('state') != login_session['state']:
        response = make_response(json.dumps('Invalid state parameter.'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
    # Obtain authorization code from server
    code = request.data

    try:
        # Upgrade the authorization code into a credentials object to access token to server
...
def getDescription(category_id, item_id):
    ...
    if login_session.get('email') is not None:
        ...
        #photos = session.query(Item).filter_by()
